...
fn generate_zk_proof(batch: &TransactionBatch) -> Result<ProofData> {
    ...
    log::info!("Created circuit input file: {}", input_file_path);
...
fn create_batch_circuit_input(batch: &TransactionBatch) -> Result<serde_json::Value> {
    log::info!("Creating circuit input for {} system transfers with account data", batch.transactions.len());
    
    let mut amounts = Vec::new();
    let mut signature_first_bytes = Vec::new();
    let mut from_balances_before = Vec::new();
    let mut from_balances_after = Vec::new();
    ...
    for (i, tx) in batch.transactions.iter().enumerate() {
        ...
        signature_first_bytes.push(sig_first_byte.to_string());
        from_balances_before.push(balance_before.to_string());
        from_balances_after.push(balance_after.to_string());
    ...
    while amounts.len() < 3 {
        ...
        signature_first_bytes.push("1".to_string()); 
        from_balances_before.push("0".to_string()); 
        from_balances_after.push("0".to_string());  
    }
    
    Ok(serde_json::json!({
        "amounts": amounts,
        "signature_first_bytes": signature_first_bytes,
        "from_balances_before": from_balances_before,
        "from_balances_after": from_balances_after
    }))
}
